...
namespace Vibration {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const mavros_msgs::Vibration::_header_type& val);
  ...
  MDArray_T get_arr_geometry_msgs_Vector3(MDFactory_T& factory, const mavros_msgs::Vibration::_vibration_type& val);
  //----------------------------------------------------------------------------
  void copy_from_arr_std_msgs_Header(std_msgs::Header& val, const matlab::data::StructArray& arr) {
  ...
  void copy_from_arr_geometry_msgs_Vector3(geometry_msgs::Vector3& val, const matlab::data::StructArray& arr) {
    ...
    try {
        const matlab::data::TypedArray<double> _vibrationx_arr = arr[0]["x"];
        val.x = _vibrationx_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
    ...
    try {
        const matlab::data::TypedArray<double> _vibrationy_arr = arr[0]["y"];
        val.y = _vibrationy_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
    ...
    try {
        const matlab::data::TypedArray<double> _vibrationz_arr = arr[0]["z"];
        val.z = _vibrationz_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
  ...
  //----------------------------------------------------------------------------
  MDArray_T get_arr_geometry_msgs_Vector3(MDFactory_T& factory, const mavros_msgs::Vibration::_vibration_type& val) {
    auto _vibrationoutArray = factory.createStructArray({1,1},{"x","y","z"});
    // _geometry_msgs_Vector3.x
    _vibrationoutArray[0]["x"] = factory.createScalar(val.x);
    // _geometry_msgs_Vector3.y
    _vibrationoutArray[0]["y"] = factory.createScalar(val.y);
    // _geometry_msgs_Vector3.z
    _vibrationoutArray[0]["z"] = factory.createScalar(val.z);
    return std::move(_vibrationoutArray);
  }
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<mavros_msgs::Vibration>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //vibration
        const matlab::data::StructArray vibration_arr = arr[0]["vibration"];
        copy_from_arr_geometry_msgs_Vector3(msg->vibration,vibration_arr);
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'vibration' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'vibration' is wrong type; expected a struct.");
    }
    try {
        //clipping
        const matlab::data::TypedArray<float> clipping_arr = arr[0]["clipping"];
        size_t nelem = 3;
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const mavros_msgs::Vibration>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","vibration","clipping"});
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // vibration
    outArray[0]["vibration"] = get_arr_geometry_msgs_Vector3(factory, msg->vibration);
    // clipping
    outArray[0]["clipping"] = factory.createArray<mavros_msgs::Vibration::_clipping_type::const_iterator, float>({1, msg->clipping.size()}, msg->clipping.begin(), msg->clipping.end());
