...
namespace RCIn {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const mavros_msgs::RCIn::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<mavros_msgs::RCIn>& msg, const matlab::data::StructArray arr) {
    try {
        //header
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const mavros_msgs::RCIn>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","rssi","channels"});
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // rssi
    outArray[0]["rssi"] = factory.createScalar(msg->rssi);
    // channels
    outArray[0]["channels"] = factory.createArray<mavros_msgs::RCIn::_channels_type::const_iterator, uint16_t>({1, msg->channels.size()}, msg->channels.begin(), msg->channels.end());
