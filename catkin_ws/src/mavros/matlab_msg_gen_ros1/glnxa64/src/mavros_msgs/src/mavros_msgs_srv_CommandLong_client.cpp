...
// Service Client for mavros_msgs/CommandLongRequest
...
namespace CommandLong_Request {
MAVROS_MSGS_EXPORT void copy_from_arr(mavros_msgs::CommandLong::Request& request,
...
class MAVROS_MSGS_EXPORT mavros_msgs_srv_CommandLong_client
    ...
    std::map<int64_t, mavros_msgs::CommandLong::Request> mRequestPool;
    mavros_msgs::CommandLong::Request mNextRequest;
  public:
    mavros_msgs_srv_CommandLong_client()
        : MATLABSvcClientInterface()
        , mNextRequest() {
    }
    virtual ~mavros_msgs_srv_CommandLong_client() {
    }
    virtual intptr_t createSvcClient(const std::string& srv_name,
        ...
        mClient = std::make_shared<ros::ServiceClient>(
            theNode->serviceClient<mavros_msgs::CommandLong>(srv_name));
    ...
    virtual bool send_request(void* sd,
        ...
        if (ret) {
            ...
            auto outArray =
                mavros_msgs::matlabhelper::CommandLong_Response::get_arr(mFactory, svc.response);
            if (sd != NULL) {
    ...
    virtual bool addRequestToPool(matlab::data::Array arr) {
        mavros_msgs::CommandLong::Request req;
        mavros_msgs::matlabhelper::CommandLong_Request::copy_from_arr(req, arr);
        mRequestPool.insert(
                {mavros_msgs::matlabhelper::CommandLong_Request::get_requestId_from_arr(arr), 
                         req});
        return true;
    }
...
CLASS_LOADER_REGISTER_CLASS(mavros_msgs_srv_CommandLong_client, MATLABSvcClientInterface)
#ifdef _MSC_VER
#pragma warning(pop)
#else
