...
// Service Client for mavros_msgs/ParamGetRequest
...
MAVROS_MSGS_EXPORT void copy_from_arr(mavros_msgs::ParamGet::Request& request,
...
MAVROS_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory,
                                         const mavros_msgs::ParamGet::Response& response);
} // namespace ParamGet_Response
} // namespace matlabhelper
} // namespace mavros_msgs
class MAVROS_MSGS_EXPORT mavros_msgs_srv_ParamGet_client
    ...
    std::map<int64_t, mavros_msgs::ParamGet::Request> mRequestPool;
    mavros_msgs::ParamGet::Request mNextRequest;
  public:
    mavros_msgs_srv_ParamGet_client()
        : MATLABSvcClientInterface()
        , mNextRequest() {
    }
    virtual ~mavros_msgs_srv_ParamGet_client() {
    }
    virtual intptr_t createSvcClient(const std::string& srv_name,
        ...
        mClient = std::make_shared<ros::ServiceClient>(
            theNode->serviceClient<mavros_msgs::ParamGet>(srv_name));
    ...
    virtual bool send_request(void* sd,
        ...
        if (ret) {
            ...
            auto outArray =
                mavros_msgs::matlabhelper::ParamGet_Response::get_arr(mFactory, svc.response);
            if (sd != NULL) {
    ...
    virtual bool addRequestToPool(matlab::data::Array arr) {
        ...
        mavros_msgs::matlabhelper::ParamGet_Request::copy_from_arr(req, arr);
        mRequestPool.insert(
                {mavros_msgs::matlabhelper::ParamGet_Request::get_requestId_from_arr(arr), 
                         req});
        return true;
    }
...
CLASS_LOADER_REGISTER_CLASS(mavros_msgs_srv_ParamGet_client, MATLABSvcClientInterface)
#ifdef _MSC_VER
#pragma warning(pop)
#else
