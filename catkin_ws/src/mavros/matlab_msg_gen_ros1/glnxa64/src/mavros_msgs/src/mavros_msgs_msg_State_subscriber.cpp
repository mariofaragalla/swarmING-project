...
// Subscriber for mavros_msgs/State
...
namespace State {
  MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const mavros_msgs::State>& msg);
} //namespace State
} //namespace matlabhelper
} //namespace mavros_msgs
class MAVROS_MSGS_EXPORT mavros_msgs_msg_State_subscriber : public MATLABSubscriberInterface {
  ...
  public:
    ...
    void callback(const mavros_msgs::State::ConstPtr& msg){
      if(mSd){
        auto outArray = mavros_msgs::matlabhelper::State::get_arr(mFactory, msg);
        appendAndSendToMATLAB(mSd, mSendDataToMATLABFunc, outArray);
      }
    }
    mavros_msgs_msg_State_subscriber()
        : MATLABSubscriberInterface() {
    }
    virtual ~mavros_msgs_msg_State_subscriber() {
    }
    virtual intptr_t createSubscription(const std::string& topic_name,
        ...
        mSub = std::make_shared<ros::Subscriber>(
                n->subscribe<mavros_msgs::State>(
                            topic_name, 
                            buffer_size, 
                            &mavros_msgs_msg_State_subscriber::callback, 
                            this,
...
CLASS_LOADER_REGISTER_CLASS(mavros_msgs_msg_State_subscriber, MATLABSubscriberInterface)
#ifdef _MSC_VER
#pragma warning(pop)
#else
