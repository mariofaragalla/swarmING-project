...
// Common copy functions for mavros_msgs/FileReadRequest
...
namespace FileReadRequest {
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<mavros_msgs::FileReadRequest>& msg, const matlab::data::StructArray arr) {
    try {
        //file_path
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const mavros_msgs::FileReadRequest>& msg) {
    auto outArray = factory.createStructArray({1,1},{"file_path","offset","size"});
    // file_path
