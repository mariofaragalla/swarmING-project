...
namespace BatteryStatus {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const mavros_msgs::BatteryStatus::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<mavros_msgs::BatteryStatus>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //voltage
        const matlab::data::TypedArray<float> voltage_arr = arr[0]["voltage"];
    ...
    try {
        //current
        const matlab::data::TypedArray<float> current_arr = arr[0]["current"];
    ...
    try {
        //remaining
        const matlab::data::TypedArray<float> remaining_arr = arr[0]["remaining"];
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const mavros_msgs::BatteryStatus>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","voltage","current","remaining"});
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // voltage
    outArray[0]["voltage"] = factory.createScalar(msg->voltage);
    // current
    outArray[0]["current"] = factory.createScalar(msg->current);
    // remaining
    outArray[0]["remaining"] = factory.createScalar(msg->remaining);
