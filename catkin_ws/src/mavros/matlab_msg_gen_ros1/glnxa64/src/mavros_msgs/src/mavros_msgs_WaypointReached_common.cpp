...
namespace WaypointReached {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const mavros_msgs::WaypointReached::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<mavros_msgs::WaypointReached>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //wp_seq
        const matlab::data::TypedArray<uint16_t> wp_seq_arr = arr[0]["wp_seq"];
        msg->wp_seq = wp_seq_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'wp_seq' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'wp_seq' is wrong type; expected a uint16.");
    }
  }
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const mavros_msgs::WaypointReached>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","wp_seq"});
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // wp_seq
    outArray[0]["wp_seq"] = factory.createScalar(msg->wp_seq);
