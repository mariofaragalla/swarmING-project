...
// Subscriber for mavros_msgs/RCIn
...
namespace RCIn {
  MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const mavros_msgs::RCIn>& msg);
} //namespace RCIn
} //namespace matlabhelper
} //namespace mavros_msgs
class MAVROS_MSGS_EXPORT mavros_msgs_msg_RCIn_subscriber : public MATLABSubscriberInterface {
  ...
  public:
    ...
    void callback(const mavros_msgs::RCIn::ConstPtr& msg){
      if(mSd){
        auto outArray = mavros_msgs::matlabhelper::RCIn::get_arr(mFactory, msg);
        appendAndSendToMATLAB(mSd, mSendDataToMATLABFunc, outArray);
      }
    }
    mavros_msgs_msg_RCIn_subscriber()
        : MATLABSubscriberInterface() {
    }
    virtual ~mavros_msgs_msg_RCIn_subscriber() {
    }
    virtual intptr_t createSubscription(const std::string& topic_name,
        ...
        mSub = std::make_shared<ros::Subscriber>(
                n->subscribe<mavros_msgs::RCIn>(
                            topic_name, 
                            buffer_size, 
                            &mavros_msgs_msg_RCIn_subscriber::callback, 
                            this,
...
CLASS_LOADER_REGISTER_CLASS(mavros_msgs_msg_RCIn_subscriber, MATLABSubscriberInterface)
#ifdef _MSC_VER
#pragma warning(pop)
#else
