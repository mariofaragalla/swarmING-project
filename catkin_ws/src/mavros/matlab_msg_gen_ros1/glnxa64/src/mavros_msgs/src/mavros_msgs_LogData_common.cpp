...
namespace LogData {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const mavros_msgs::LogData::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<mavros_msgs::LogData>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //offset
        const matlab::data::TypedArray<uint32_t> offset_arr = arr[0]["offset"];
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const mavros_msgs::LogData>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","id","offset","data"});
    // header
    ...
    // data
    outArray[0]["data"] = factory.createArray<mavros_msgs::LogData::_data_type::const_iterator, uint8_t>({1, msg->data.size()}, msg->data.begin(), msg->data.end());
