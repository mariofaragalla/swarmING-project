...
namespace StatusText {
  ...
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const mavros_msgs::StatusText::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<mavros_msgs::StatusText>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //severity
        const matlab::data::TypedArray<uint8_t> severity_arr = arr[0]["severity"];
    ...
    try {
        //text
        const matlab::data::CharArray text_arr = arr[0]["text"];
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const mavros_msgs::StatusText>& msg) {
    auto outArray = factory.createStructArray({1,1},{"EMERGENCY","ALERT","CRITICAL","ERROR","WARNING","NOTICE","INFO","DEBUG","header","severity","text"});
    // EMERGENCY
    outArray[0]["EMERGENCY"] = factory.createScalar(static_cast<uint8_t>(msg->EMERGENCY));
    // ALERT
    outArray[0]["ALERT"] = factory.createScalar(static_cast<uint8_t>(msg->ALERT));
    // CRITICAL
    outArray[0]["CRITICAL"] = factory.createScalar(static_cast<uint8_t>(msg->CRITICAL));
    // ERROR
    outArray[0]["ERROR"] = factory.createScalar(static_cast<uint8_t>(msg->ERROR));
    // WARNING
    outArray[0]["WARNING"] = factory.createScalar(static_cast<uint8_t>(msg->WARNING));
    // NOTICE
    outArray[0]["NOTICE"] = factory.createScalar(static_cast<uint8_t>(msg->NOTICE));
    // INFO
    outArray[0]["INFO"] = factory.createScalar(static_cast<uint8_t>(msg->INFO));
    // DEBUG
    outArray[0]["DEBUG"] = factory.createScalar(static_cast<uint8_t>(msg->DEBUG));
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // severity
    outArray[0]["severity"] = factory.createScalar(msg->severity);
    // text
    outArray[0]["text"] = factory.createCharArray(msg->text);
