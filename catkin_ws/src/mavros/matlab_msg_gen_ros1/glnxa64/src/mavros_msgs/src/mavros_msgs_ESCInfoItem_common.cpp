...
namespace ESCInfoItem {
  void copy_from_arr_std_msgs_Header(std_msgs::Header& val, const matlab::data::StructArray& arr);
  MDArray_T get_arr_std_msgs_Header(MDFactory_T& factory, const mavros_msgs::ESCInfoItem::_header_type& val);
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<mavros_msgs::ESCInfoItem>& msg, const matlab::data::StructArray arr) {
    try {
        //header
    ...
    try {
        //failure_flags
        const matlab::data::TypedArray<uint16_t> failure_flags_arr = arr[0]["failure_flags"];
    ...
    try {
        //error_count
        const matlab::data::TypedArray<uint32_t> error_count_arr = arr[0]["error_count"];
    ...
    try {
        //temperature
        const matlab::data::TypedArray<uint8_t> temperature_arr = arr[0]["temperature"];
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const mavros_msgs::ESCInfoItem>& msg) {
    auto outArray = factory.createStructArray({1,1},{"header","failure_flags","error_count","temperature"});
    // header
    outArray[0]["header"] = get_arr_std_msgs_Header(factory, msg->header);
    // failure_flags
    outArray[0]["failure_flags"] = factory.createScalar(msg->failure_flags);
    // error_count
    outArray[0]["error_count"] = factory.createScalar(msg->error_count);
    // temperature
