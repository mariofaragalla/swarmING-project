...
// Service Client for mavros_msgs/FileWriteRequest
...
namespace FileWrite_Request {
MAVROS_MSGS_EXPORT void copy_from_arr(mavros_msgs::FileWrite::Request& request,
...
MAVROS_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory,
                                         const mavros_msgs::FileWrite::Response& response);
} // namespace FileWrite_Response
} // namespace matlabhelper
} // namespace mavros_msgs
class MAVROS_MSGS_EXPORT mavros_msgs_srv_FileWrite_client
    ...
    std::map<int64_t, mavros_msgs::FileWrite::Request> mRequestPool;
    mavros_msgs::FileWrite::Request mNextRequest;
  public:
    mavros_msgs_srv_FileWrite_client()
        : MATLABSvcClientInterface()
        , mNextRequest() {
    }
    virtual ~mavros_msgs_srv_FileWrite_client() {
    }
    virtual intptr_t createSvcClient(const std::string& srv_name,
        ...
        mClient = std::make_shared<ros::ServiceClient>(
            theNode->serviceClient<mavros_msgs::FileWrite>(srv_name));
    ...
    virtual bool send_request(void* sd,
        ...
        if (ret) {
            ...
            auto outArray =
                mavros_msgs::matlabhelper::FileWrite_Response::get_arr(mFactory, svc.response);
            if (sd != NULL) {
    ...
    virtual bool addRequestToPool(matlab::data::Array arr) {
        mavros_msgs::FileWrite::Request req;
        mavros_msgs::matlabhelper::FileWrite_Request::copy_from_arr(req, arr);
        mRequestPool.insert(
                {mavros_msgs::matlabhelper::FileWrite_Request::get_requestId_from_arr(arr), 
                         req});
        return true;
    }
...
CLASS_LOADER_REGISTER_CLASS(mavros_msgs_srv_FileWrite_client, MATLABSvcClientInterface)
#ifdef _MSC_VER
#pragma warning(pop)
#else
