...
namespace FileWrite_Request {
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT void copy_from_arr(mavros_msgs::FileWrite::Request& msg, const matlab::data::StructArray arr) {
    try {
        //file_path
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const mavros_msgs::FileWrite::Request& msg) {
    ...
    // data
    outArray[0]["data"] = factory.createArray<mavros_msgs::FileWrite::Request::_data_type::const_iterator, uint8_t>({1, msg.data.size()}, msg.data.begin(), msg.data.end());
