...
namespace WaypointList {
  ...
  MDArray_T get_arr_mavros_msgs_Waypoint_var_arr(MDFactory_T& factory, const mavros_msgs::WaypointList::_waypoints_type& val);
  //----------------------------------------------------------------------------
  void copy_from_arr_mavros_msgs_Waypoint_var_arr(mavros_msgs::Waypoint& val, const matlab::data::Struct& arr) {
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT void copy_from_arr(boost::shared_ptr<mavros_msgs::WaypointList>& msg, const matlab::data::StructArray arr) {
    try {
        //current_seq
        const matlab::data::TypedArray<uint16_t> current_seq_arr = arr[0]["current_seq"];
        msg->current_seq = current_seq_arr[0];
    } catch (matlab::data::InvalidFieldNameException&) {
        throw std::invalid_argument("Field 'current_seq' is missing.");
    } catch (matlab::data::TypeMismatchException&) {
        throw std::invalid_argument("Field 'current_seq' is wrong type; expected a uint16.");
    }
    try {
        //waypoints
  ...
  //----------------------------------------------------------------------------
  MAVROS_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const boost::shared_ptr<const mavros_msgs::WaypointList>& msg) {
    auto outArray = factory.createStructArray({1,1},{"current_seq","waypoints"});
    // current_seq
    outArray[0]["current_seq"] = factory.createScalar(msg->current_seq);
    // waypoints
