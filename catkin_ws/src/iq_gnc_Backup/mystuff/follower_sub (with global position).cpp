...
void drone1_gpsSub_cb(const sensor_msgs::NavSatFix::ConstPtr& msg){
	...
	if(isMapOriginSet){
		...
		if(altitude_reached){
			// feeh moshkela hena fl error shklo msh mzboot
			ROS_INFO("Start following");
			if(ros_namespace_l == "/drone2"){
				ROS_INFO((ros_namespace_l).c_str());
				go_to_goalPos(-2,0,3);
			}
			if(ros_namespace_l == "/drone3"){
				ROS_INFO((ros_namespace_l).c_str());
...
/*
			...
			geometry_msgs::PoseStamped error_wp = difference_bet_2_wps(difference_wp, base_difference_wp);
			...
			if(abs(error_wp.pose.position.x) > 0.01){
				...
				for(int i = 0; i<100; i++){
					...
					set_destination(0, error_wp.pose.position.x,  error_wp.pose.position.z, 0);
