...
void gps_data_cb(const sensor_msgs::NavSatFix::ConstPtr& msg){
	...
	sensor_msgs::NavSatFix gps_data_g = *msg;
...
void set_map_origin(float LA, float LO , float ALT){
		...
		ROS_INFO("Map origin set to: [%f, %f, %f]", map_origin_gps.latitude, map_origin_gps.longitude, map_origin_gps.altitude);
		// isMapOriginSet = true;
}
...
void go_to_goalPos(float x, float y, float z) {  //global
	/*
	mavros_msgs::GlobalPositionTarget target_global_position;

	// ros::Rate rate(10);
	// target_global_position.header.frame_id = "base_link";
	target_global_position.latitude = x*pow(10,-5) + map_origin_gps.latitude; 
	target_global_position.longitude = y*pow(10,-5) + map_origin_gps.longitude; 
	target_global_position.altitude = z + map_origin_gps.altitude; 
	target_global_position.yaw = 0;
	*/


	geographic_msgs::GeoPoseStamped	target_global_position;

	target_global_position.header.stamp = ros::Time::now();
    ...
    ROS_INFO("Go To: LA: %f , LO: %f , ALT: %f", target_global_position.pose.position.latitude, target_global_position.pose.position.longitude ,target_global_position.pose.position.altitude);
    // ros::spinOnce();
	// goal_pos_pub.publish(target_global_position);

}

// void circle (float x_r,float raduis){

// 	int8_t sign; 
// 	if(x_r == raduis){
// 		sign = -1;
// 	} else if(x_r == -1*raduis){
// 		sign = -1*sign;
// 	}
// 	go_to_goalPos(x_r, sign*sqrt(pow(raduis,2)-pow(x_r,2)),3);
...
int init_publisher_subscriber(ros::NodeHandle controlnode)
{
	...
	curr_gps_data = controlnode.subscribe<sensor_msgs::NavSatFix>((ros_namespace +"/mavros/global_position/raw/fix").c_str(), 10, gps_data_cb);
