...
void set_map_origin(){
	...
	if(current_state_g.mode == "STABILIZE" && ros_namespace == "/drone1"){
		map_origin_gps.latitude = gps_position.latitude ;
		map_origin_gps.longitude = gps_position.longitude;
		map_origin_gps.altitude = gps_position.altitude ;

		ROS_INFO("Map origin set to: [%.2f, %.2f, %.2f]", map_origin_gps.latitude, map_origin_gps.longitude, map_origin_gps.altitude);
	}
}
...
void convertLLA2xyz(float longitude, float latitude , float altitude){
...
int init_publisher_subscriber(ros::NodeHandle controlnode)
{
	...
	// curr_gps_pub = controlnode.advertise<mavros_msgs::GlobalPositionTarget>((ros_namespace +"mavros/setpoint_position/global").c_str(), 10);
