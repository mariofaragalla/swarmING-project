...
int main(int argc, char** argv)
{
	...
	std::string ros_namespace;
  	...
  	dsub.getParam("namespace", ros_namespace);
	...
	while(ros::ok())		//while the program is not broken (I think no ctrl + C is pressed)
	{
		...
		//{
				...
				if(!check_waypoint_reached(.1))	{
				   ...
				   drone1Pos = dsub.subscribe<geometry_msgs::PoseStamped>("/drone1/mavros/local_position/pose", 1, drone1_pos_cb);
				}
				else {
