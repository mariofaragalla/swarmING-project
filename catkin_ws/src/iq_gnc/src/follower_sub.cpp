...
/* leader drone quaternions q0, q1, q2, q3
float drone1_q0;
float drone1_q1;
float drone1_q2;
float drone1_q3;
...
mavros_msgs::State drone1_state;	// leader drone state: 'LOITER','GUIDED','STABILIZE'
geometry_msgs::PoseStamped drone1_pose_g;	// leader drone position recieved from gps sensor (LO, LA , ALT)
mavros_msgs::GlobalPositionTarget curr_gps_g;

geometry_msgs::PoseStamped current_global_position;	//the global position of the current drone (2 or 3)
geometry_msgs::PoseStamped prev_drone1_global_position;
geometry_msgs::PoseStamped drone1_global_position;	//leader drone position in xyz
...
void drone1_gpsSub_cb(const sensor_msgs::NavSatFix::ConstPtr& msg){
	...
	if(drone1_state.mode == "STABILIZE" && drone1_gps_g.altitude != 0 && !isMapOriginSet){	
		...
		current_global_position = get_global_position();
		desired_posDiff_wp = difference_bet_2_wps(drone1_global_position, current_global_position); 
		...
		// ROS_INFO("current_global_position: %f %f %f", current_global_position.pose.position.x , current_global_position.pose.position.y, current_global_position.pose.position.z);
	...
	if(isMapOriginSet){
		...
		current_global_position = get_global_position();
		// ROS_INFO("%s LLA2XYZ: [%.2f, %.2f, %.2f]",  ros_namespace_l.c_str(), current_global_position.pose.position.x, current_global_position.pose.position.y, current_global_position.pose.position.z);
		...
		if(drone1_global_position.pose.position.z > 0.5){
			...
			if(abs(drone1_global_position.pose.position.z - prev_drone1_global_position.pose.position.z) <= 0.01){
				...
				if(!takeoff_flag){
					...
					while(current_global_position.pose.position.z < 0.5 && i<1000){
					...
					takeoff_flag = true;
					if(current_global_position.pose.position.z > 0.5){
						altitude_reached = true;
					} else {
						takeoff_flag = false;
						altitude_reached = false;
		...
		if(altitude_reached && current_state_g.mode != "LAND"){
			...
			if(ros_namespace_l == "/drone2" && !drone2_first_pos){
				...
				go_to_goalPos(-2,-2,3, current_global_position.header.stamp);
			...
			geometry_msgs::PoseStamped posDiff_wp = difference_bet_2_wps(drone1_global_position, current_global_position); 
	 ...
	 //try impedendence controller
		...
		if(my_time[0] > 0 && my_time[1] > 0){
			...
			// if(abs(drone1_global_position.pose.position.x - prev_drone1_global_position.pose.position.x) > 0.01 || abs(drone1_global_position.pose.position.y - prev_drone1_global_position.pose.position.y) != 0 ) {
				...
				vel = vel_control_input_PID(Fext_arr ,abs(my_time[1]-my_time[0]), sampleCount)	;
				// vel = acc2vel(Fext_arr[sampleCount],curr_gps_vel_g,abs(my_time[1]-my_time[0]));
				
				reach_goalVel(vel);
				
			} else{

				go_to_goalPos(current_global_position.pose.position.x + posError_wp.pose.position.x ,  current_global_position.pose.position.y  +  posError_wp.pose.position.y ,  current_global_position.pose.position.z + posError_wp.pose.position.z, drone1_global_position.header.stamp);
